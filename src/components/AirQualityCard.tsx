...
const AQI_RING_RADIUS = 38

// EU AQI breakpoints (μg/m³)
function buildPollutants(pm25: number, pm10: number, no2: number): PollutantConfig[] {
  return [
    {
      label: "PM2.5",
      value: pm25,
...
export function AirQualityCard({ aqi, pm25, pm10, no2 }: AirQualityCardProps) {
  ...
  const circumference = 2 * Math.PI * AQI_RING_RADIUS
  const pollutants = buildPollutants(pm25, pm10, no2)
      ...
      <div className="relative z-10 flex flex-col flex-1">
        ...
        <CardContent className="flex flex-col flex-1">
          <div className="flex flex-col flex-1">
            ...
            {/* Pollutant details pinned to bottom */}
            <div className="grid grid-cols-3 gap-2 text-xs mt-auto pt-4">
              {pollutants.map((cfg) => {
                const [, pColor, status] = findBand(cfg.bands, cfg.value)
                return (
